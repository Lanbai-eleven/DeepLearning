...
def batchnorm_backward_alt(dout, cache):
    ...
    dmean = np.sum(-dx_norm/np.sqrt(x_var+eps),axis=0) + dvar*np.sum(-2 * (x -x_mean), axis=0)/m
    dx = dx_norm/np.sqrt(x_var+eps) + dvar*2*(x-x_mean)/m + dmean/m
