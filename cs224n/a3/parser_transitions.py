...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        batch_size = min(batch_size, len(unfinished_parses))
        minibatch_parses = unfinished_parses[:batch_size]
        transitions = model.predict(minibatch_parses)
        for i, transition in enumerate(transitions):
            minibatch_parses[i].parse_step(transition)          
            
        unfinished_parses = [partial_parse for partial_parse in unfinished_parses
