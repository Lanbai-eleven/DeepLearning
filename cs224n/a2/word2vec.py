...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs -- the gradient with respect to all the outside word vectors
                    ...
                    (dJ / dU)
    """
    output = outsideVectors.dot(centerWordVec)
    pred = softmax(output)
    loss = -np.log(pred[outsideWordIdx])

    pred[outsideWordIdx] -= 1
    gradCenterVec = outsideVectors.T.dot(pred)
    gradOutsideVecs = pred.reshape(-1,1).dot(centerWordVec.reshape(1,-1))
...
def negSamplingLossAndGradient(
    ...
    negSampleVecs = outsideVectors[negSampleWordIndices]
    sigmoid_center = sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec))
    sigmoid_negSamples = sigmoid(negSampleVecs.dot(centerWordVec))
    loss = -np.log(sigmoid_center) - np.sum(np.log(1-sigmoid_negSamples))
    gradCenterVec = (sigmoid_center-1)*outsideVectors[outsideWordIdx] \
        +np.sum(sigmoid_negSamples.reshape(-1,1)*negSampleVecs, axis=0)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (sigmoid_center-1)*centerWordVec
    for i, negsampleidx in enumerate(negSampleWordIndices):
        gradOutsideVecs[negsampleidx] += sigmoid_negSamples[i]*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        curloss,curgradCenterVecs,curgradOutVecs = \
        word2vecLossAndGradient(currentCenterWordVec,
                                word2Ind[outsideWord],
                                outsideVectors,
                                dataset)
        loss += curloss
        gradCenterVecs[currentCenterWordIdx] += curgradCenterVecs
